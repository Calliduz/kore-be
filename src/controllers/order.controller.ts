...
export const addOrderItems = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    for (const item of orderItems) {
      ...
      if (!product.isActive) {
        ApiResponse.badRequest(`Product is not available: ${item.name}`).send(
          res
        );
        return;
      }
...
export const updateOrderToPaid = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    ApiResponse.success({ order: updatedOrder }, "Order marked as paid").send(
      res
    );
  }
);
...
export const getAllOrders = asyncHandler(
  async (req: AuthRequest, res: Response) => {
    ...
    ApiResponse.success({ orders }, "All orders retrieved successfully").send(
      res
    );
  }
);
